...
wonderlisp 自身も `boot.lisp` を用いて初期化される Lisp 処理系なので、wonderlisp 自身を wonderlisp で動かせる。Rosetta Lisp シリーズはVMがあれど S 式を順に逐次実行していく処理系なので、厳密にセルフホスティングできているかの検証は難しい。
